...
async function runMigrationOnce(db, name, fn) {
  ...
  await fn();
  // OR IGNORE：并发冷启动的实例可能同时通过上面的检查，落档撞主键不算失败
  await db.execute({
...
async function migrateOrdersSchema(db) {
  ...
  // 门票验票链接存档（二维码内容）：出票时写入，存量订单按规范域名回填
  await addColumnIfMissing(db, 'orders', 'verify_url', 'TEXT');
  await runMigrationOnce(db, 'orders_verify_url_backfill_v1', async function () {
    var base = String(env('SITE_URL') || 'https://www.rising2026.com').replace(/\/$/, '');
    await db.execute({
      sql: "UPDATE orders SET verify_url = ? || merchant_order_no WHERE verify_url IS NULL OR verify_url = ''",
      args: [base + '/v/']
    });
  });

  // 门票二维码图片存档（PNG base64，单张约 2~4KB）：出票时写入，存量订单按验票链接补生成
  await addColumnIfMissing(db, 'orders', 'qr_png_base64', 'TEXT');
  await runMigrationOnce(db, 'orders_qr_png_backfill_v1', async function () {
    var pending = await db.execute(`
      SELECT merchant_order_no, verify_url FROM orders
      WHERE (qr_png_base64 IS NULL OR qr_png_base64 = '')
        AND verify_url IS NOT NULL AND verify_url != ''
    `);
    if (!pending.rows.length) return;
    var generateTicketQrBase64 = require('./ticket-qr').generateTicketQrBase64;
    var stmts = [];
    for (var i = 0; i < pending.rows.length; i++) {
      var row = pending.rows[i];
      stmts.push({
        sql: 'UPDATE orders SET qr_png_base64 = ? WHERE merchant_order_no = ?',
        args: [await generateTicketQrBase64(row.verify_url), String(row.merchant_order_no)]
      });
    }
    // 分批写入：每批一次网络往返，避免逐条 UPDATE 拖慢冷启动
    for (var j = 0; j < stmts.length; j += 100) {
      await db.batch(stmts.slice(j, j + 100), 'write');
    }
  });
...
var SCHEMA_SETUP_MARK = 'schema_setup_v7'; // v7: orders 增加 qr_png_base64（门票二维码图片存档）；v6: verify_url + nominees 最终合成海报字段
...
async function doEnsureSchema() {
  ...
  // 最终合成海报（人像 + 模板，浏览器合成后回传存档）：老库通过 ALTER 补齐
  await addColumnIfMissing(db, 'nominees', 'final_poster_base64', 'TEXT');
  await addColumnIfMissing(db, 'nominees', 'final_poster_mime', 'TEXT');
  await addColumnIfMissing(db, 'nominees', 'final_poster_at', 'TEXT');
  ...
  // 提名嘉宾免费入场票票号（关联 orders.merchant_order_no）：老库通过 ALTER 补齐
  await addColumnIfMissing(db, 'nominees', 'ticket_order_no', 'TEXT');
  ...
  await db.execute({
    sql: `
      INSERT INTO orders (
        ...
        verify_url, qr_png_base64, client_ip, user_agent, status, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)
    `,
    args: [
...
// 后台详情用：只取二维码存档相关字段，不拖整行
async function getOrderQrArchive(merchantOrderNo) {
  await ensureSchema();
  var db = getClient();
  var result = await db.execute({
    sql: 'SELECT merchant_order_no, verify_url, qr_png_base64 FROM orders WHERE merchant_order_no = ? LIMIT 1',
    args: [merchantOrderNo]
  });
  if (!result.rows.length) return null;
  return result.rows[0];
...
// 二维码图片补存档（部署窗口期漏写的订单自愈用）：首写优先；
// 这类订单 verify_url 往往也是空的，随补档一并回写（已有值不覆盖）
async function saveOrderTicketQr(merchantOrderNo, base64, verifyUrl) {
  await ensureSchema();
  var db = getClient();
  var result = await db.execute({
    sql: `
      UPDATE orders SET qr_png_base64 = ?,
        verify_url = CASE WHEN verify_url IS NULL OR verify_url = '' THEN ? ELSE verify_url END
      WHERE merchant_order_no = ? AND (qr_png_base64 IS NULL OR qr_png_base64 = '')
    `,
    args: [base64, verifyUrl || '', merchantOrderNo]
  });
  return result.rowsAffected > 0;
}
...
async function listOrdersForAdmin() {
  ...
  var result = await db.execute(`
    SELECT
      merchant_order_no, transaction_no, name, nickname, company, title,
      phone, email, wechat, industry, invite,
      ticket, ticket_name, price, paid_money, pay_method, status,
      verify_url, created_at, paid_at,
      (SELECT COUNT(*) FROM ticket_verifications tv
        WHERE tv.merchant_order_no = orders.merchant_order_no) AS verify_count
    FROM orders
    WHERE status = 'paid'
...
async function listNomineesBrief() {
  ...
  var result = await db.execute(`
    SELECT
      ...
      CASE WHEN final_poster_base64 IS NOT NULL AND final_poster_base64 != '' THEN 1 ELSE 0 END AS has_final_poster,
      created_at
    FROM nominees
    ORDER BY id DESC
  `);
...
// 海报存档状态轻查询：不拉 base64 大字段
async function getNomineePosterState(id) {
  await ensureSchema();
  var db = getClient();
  var result = await db.execute({
    sql: `
      SELECT id,
        CASE WHEN poster_base64 IS NOT NULL AND poster_base64 != '' THEN 1 ELSE 0 END AS has_portrait,
        CASE WHEN final_poster_base64 IS NOT NULL AND final_poster_base64 != '' THEN 1 ELSE 0 END AS has_final_poster
...
// 最终合成海报存档：首写优先（WHERE 限定空值，并发/重复回传不会覆盖首份存档）；
// force=true（仅管理端鉴权后）允许覆盖重写，用于纠正错档
async function saveNomineeFinalPoster(id, base64, mime, force) {
  await ensureSchema();
  var db = getClient();
  var guard = force ? '' : " AND (final_poster_base64 IS NULL OR final_poster_base64 = '')";
  var result = await db.execute({
    sql: 'UPDATE nominees SET final_poster_base64 = ?, final_poster_mime = ?, final_poster_at = ? WHERE id = ?' + guard,
    args: [base64, mime || 'image/jpeg', nowChinaSql(), id]
  });
  return result.rowsAffected > 0;
}
  ...
  var result = await db.execute({
    sql: `
      SELECT
        id, name, nickname, company, title, phone, email, wechat, address,
        attend_ceremony, accept_interview, interview_city, accept_ceremony_interview, auth_agreed,
        proud_decision, complex_problem, one_line_intro, ability_tag, ticket_order_no,
...
module.exports = {
  ...
  getNomineeForPoster,
  saveNomineePoster,
  getNomineePosterState,
  saveNomineeFinalPoster,
  insertNominee,
